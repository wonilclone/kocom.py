...
room_t_dic = {'00':'livingroom', '01':'room1', '02':'room2', '03':'room3', '04':'kitchen', 'hallway':'05'}
...
room_h_dic = {'livingroom':'00', 'myhome':'00', 'room1':'01', 'room2':'02', 'room3':'03', 'kitchen':'04', 'hallway':'05'}
...
def init_mqttc():
    ...
    while True:
        try:
            retry_cnt += 1
            logging.info(f"{logtxt} - try #{retry_cnt}")
        ...
        if self.type == 'serial':
            for _ in range(polling_interval+15):
                try:
                    ...
                    raise Exception('exception occurred while reading serial')
                if len(ret) != 0:
                    break
        elif self.type == 'socket':
...
def parse(hex_data):
    ...
    dest_h = hex_data[10:14]      # dest addr
    src_h = hex_data[14:18]       # source addr
    cmd_h = hex_data[18:20]       # command
...
def ac_parse(value):
    ...
    logtxt = '[MQTT Parse | Ac] value[{}], state[{}]'.format(value, state)
    if logtxt != '' and config.get('Log', 'show_recv_hex') == 'True':
        logging.info(logtxt)
...
def query(device_h, publish=False, enforce=False):
    ...
    for c in cache_data:
        if enforce:
            break
        if time.time() - c['time'] > polling_interval:  # outdated cache
            break
    ...
    # if there's no cache data within polling interval, then send query packet
    if config.get('Log', 'show_query_hex') == 'True':
...
def send_wait_response(dest, src=device_h_dic['wallpad']+'00', cmd=cmd_h_dic['state'],
                       value='0'*16, log=None, check_ack=True, publish=True):
...
def mqtt_on_message(mqttc, obj, msg):
    ...
    if 'thermo' in topic_d and 'heat_mode' in topic_d:
        ...
        value = heatmode_dic.get(command, '00') + '00' + settemp_hex + '0000000000'
    ...
    # fan preset
        ...
        if command in onoff_dic.keys():
            ...
            speed = speed_dic.get(init_fan_mode, '40')  # 기본값
        value = onoff + speed + '0'*10
        send_wait_response(dest=dev_id, value=value, log='fan')
...
def poll_state(enforce=False):
    ...
    for thread_instance in thread_list:
        if not thread_instance.is_alive():
            logging.error('[THREAD] {} is not active. restarting thread.'.format(thread_instance.name))
            # [수정] 필요 시 재시작 로직 or 종료 후 재실행
            # 여기서는 단순 로그만 남김
    ...
    for t in dev_list:
        ...
        if dev_id is not None and sub_id is not None:
            # [수정] 기존에는 query 실패 시 break -> 이제는 계속 진행
        ...
        try:
            ...
            if len(buf) == (packet_size * 2):
                ...
                if chksum_calc == chksum_buf and buf[-len(trailer_h):] == trailer_h:
                    if msg_q.full():
                        logging.error('msg_q is full. Possibly listener thread error. Consider restart.')
    ...
    while True:
        ...
        if not wait_target.empty():
            if p_ret['dest_h'] == wait_target.queue[0] and p_ret['type'] == 'ack':
                if len(ack_data) != 0:
                    logging.info("[ACK] No ack, but response packet received first. Assume ACK OK")
                    ack_q.put(d)
                    time.sleep(0.5)
                wait_q.put(p_ret)
                continue
...
if __name__ == "__main__":
    ...
    # [수정] RS485 초기 연결 시도 실패해도 exit(1) 사용하지 않음. 무한 재시도
    while True:
        if config.get('RS485', 'type') == 'serial':
            ...
            rs485 = RS485Wrapper(socket_server=config.get('RS485', 'socket_server'),
        ...
        conn = rs485.connect()
        if conn != False:
            logging.info('[RS485] connected successfully.')
            break
        else:
            logging.error('[RS485] connection error. retry in 10 seconds...')
            time.sleep(10)

    # [수정] MQTT도 실패해도 프로그램 종료하지 않고, init_mqttc() 내부에서 무한 재시도
    mqttc = init_mqttc()
